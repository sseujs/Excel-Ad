...
let CreateApiKeyDriver = class CreateApiKeyDriver {
    ...
    validateSecret(apiKeySecret) {
        ...
        if (apiKeySecret.length > constants_1.maxSecretLength || apiKeySecret.length < constants_1.minSecretLength) {
            return false;
        }
        return true;
    }
    ...
    async getDomain(args, context) {
        ...
        const parser = new spec_parser_1.SpecParser(absolutePath, {
            allowAPIKeyAuth: featureFlags_1.isApiKeyEnabled(),
            allowMultipleParameters: featureFlags_1.isMultipleParametersEnabled(),
        });
        const operations = await parser.list();
        const domains = operations
            .filter((value) => {
            var _a, _b;
            return ((_a = value.auth) === null || _a === void 0 ? void 0 : _a.type) === "apiKey" && ((_b = value.auth) === null || _b === void 0 ? void 0 : _b.name) === args.name;
        })
            .map((value) => {
            return value.server;
        })
