...
let ScriptDriver = class ScriptDriver {
    ...
    async execute(args, ctx) {
        ...
        const summaries = res.isOk()
            ? [`Successfully executed command ${envUtil_1.maskSecretValues(args.run)}`]
            : [];
...
async function executeCommand(command, projectPath, logProvider, ui, workingDirectory, env, shell, timeout, redirectTo) {
    ...
    return new Promise((resolve) => {
        ...
        logProvider.verbose(`Start to run command: "${envUtil_1.maskSecretValues(finalCmd)}" with args: ${JSON.stringify({
            shell: finalShell,
            cwd: workingDir,
        ...
        const cp = child_process_1.default.exec(finalCmd, {
            ...
            if (error) {
                ...
                resolve(teamsfx_api_1.err(convertScriptErrorToFxError(error, finalCmd)));
            }
            else {
                ...
                if (Object.keys(outputObject).length > 0)
                    logProvider.verbose(`script output env variables: ${envUtil_1.maskSecretValues(JSON.stringify(outputObject))}`);
        ...
        (_a = cp.stdout) === null || _a === void 0 ? void 0 : _a.on("data", (data) => {
            ...
            logProvider.info(` [script stdout] ${envUtil_1.maskSecretValues(str)}`);
            dataHandler(str);
        });
...
function getStderrHandler(logProvider, systemEncoding, stderrStrings, dataHandler) {
    return (data) => {
        const str = bufferToString(data, systemEncoding);
        logProvider.warning(` [script stderr] ${envUtil_1.maskSecretValues(str)}`);
        dataHandler(str);
