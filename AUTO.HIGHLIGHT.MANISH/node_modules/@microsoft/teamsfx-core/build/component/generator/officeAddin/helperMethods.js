...
class HelperMethods {
    static async downloadProjectTemplateZipFile(projectFolder, projectRepo, projectBranch) {
        ...
        const response = await node_fetch_1.default(projectTemplateZipFile, { method: "GET" });
        return new Promise((resolve, reject) => {
            if (response.body) {
                response.body
                    .pipe(fs_1.default.createWriteStream(path.resolve(projectFolder, zipFile)))
                    .on("error", (err) => {
                    reject(
    ...
    static async unzipProjectTemplate(projectFolder) {
        return new Promise((resolve, reject) => {
            ...
            const readStream = fs_1.default.createReadStream(path.resolve(`${projectFolder}/${zipFile}`));
            readStream
